...
export function AdminContactTicketsDashboard({
  ...
  const [statusFilters, setStatusFilters] = useState<Set<ContactTicketStatusValue>>(
  ...
  const [focusedTicket, setFocusedTicket] = useState<ContactTicketRow | null>(null);
  ...
  const handleStatusChange = (ticket: ContactTicketRow, nextStatus: ContactTicketStatusValue) => {
    startTransition(async () => {
      setFeedback(null);
      try {
        const response = await fetch(`/api/admin/contact-tickets/${ticket.id}`, {
          method: "PATCH",
          headers: {
        ...
        if (!response.ok) {
          ...
          throw new Error(payload?.error ?? "Mise à jour impossible pour le moment.");
        }

        const payload = (await response.json()) as { ticket: ContactTicketRow };

        setTickets((previous) =>
          previous.map((item) => (item.id === ticket.id ? payload.ticket : item)),
        );
  ...
  return (
    <div className="fr-flow">
      <div className={clsx("fr-grid-row fr-grid-row--gutters", styles.filtersRow)}>
        ...
        <div className={clsx("fr-col-12 fr-col-lg-auto", styles.filtersButtonCol)}>
          <Button
            priority="tertiary"
      ...
      {feedback && (
        <Alert
          ...
          title={feedback.status === "success" ? "Mise à jour effectuée" : "Erreur"}
        ...
        <table>
          ...
          <tbody>
            {filteredTickets.length === 0 ? (
              <tr>
                <td colSpan={5} className="fr-text--center fr-text-mention--grey">
              ...
              filteredTickets.map((ticket) => {
                ...
                const statusSeverity = CONTACT_TICKET_STATUS_BADGES[ticket.status];
                const typeLabel = CONTACT_TICKET_TYPE_LABELS[ticket.contactType];
                const createdAtLabel = dateTimeFormatter.format(new Date(ticket.createdAt));
                  ...
                  <tr key={ticket.id}>
                    ...
                    <td className="fr-text--right">
                      <div className="fr-btns-group fr-btns-group--inline fr-btns-group--icon-left fr-mb-0">
                        ...
                        <Button
                          priority={ticket.status === "PENDING" ? "primary" : "secondary"}
                          iconId={
                          ...
                          disabled={isPending}
                        >
                          {ticket.status === "PENDING" ? "Marquer traité" : "Rouvrir"}
                        </Button>
                      </div>
                    </td>
                  </tr>
        ...
        {focusedTicket ? (
          <div className="fr-flow">
            <div className="fr-grid-row fr-grid-row--gutters">
              <div className="fr-col-12 fr-col-md-6">
                <p className="fr-text--sm fr-text-mention--grey fr-mb-1v">Statut</p>
                <Badge small severity={CONTACT_TICKET_STATUS_BADGES[focusedTicket.status]}>
                  {CONTACT_TICKET_STATUS_LABELS[focusedTicket.status]}
                </Badge>
              </div>
              <div className="fr-col-12 fr-col-md-6">
                <p className="fr-text--sm fr-text-mention--grey fr-mb-1v">Type</p>
                <span>{CONTACT_TICKET_TYPE_LABELS[focusedTicket.contactType]}</span>
              </div>
            </div>

            <div className="fr-grid-row fr-grid-row--gutters">
              <div className="fr-col-12 fr-col-md-6">
                <p className="fr-text--sm fr-text-mention--grey fr-mb-1v">Nom</p>
                <p className="fr-mb-0">{focusedTicket.name}</p>
              </div>
              <div className="fr-col-12 fr-col-md-6">
                <p className="fr-text--sm fr-text-mention--grey fr-mb-1v">Fonction</p>
            ...
            <div className="fr-grid-row fr-grid-row--gutters">
              <div className="fr-col-12 fr-col-md-6">
                <p className="fr-text--sm fr-text-mention--grey fr-mb-1v">Email</p>
                <p className="fr-mb-0">{focusedTicket.email}</p>
              </div>
              <div className="fr-col-12 fr-col-md-6">
                <p className="fr-text--sm fr-text-mention--grey fr-mb-1v">
                  {focusedTicket.contactType === "COMMUNE" ? "Commune" : "Organisme"}
                </p>
                <p className="fr-mb-0">
            ...
            <div>
              <p className="fr-text--sm fr-text-mention--grey fr-mb-1v">Message</p>
              <p className="fr-mb-0 fr-text--sm" style={{ whiteSpace: "pre-wrap" }}>
            ...
            <div className="fr-grid-row fr-grid-row--gutters">
              <div className="fr-col-12 fr-col-md-6">
                <p className="fr-text--sm fr-text-mention--grey fr-mb-1v">Créé le</p>
                <p className="fr-mb-0">
                  {dateTimeFormatter.format(new Date(focusedTicket.createdAt))}
                </p>
              </div>
              <div className="fr-col-12 fr-col-md-6">
                <p className="fr-text--sm fr-text-mention--grey fr-mb-1v">Dernière mise à jour</p>
                <p className="fr-mb-0">
            ...
            <div className="fr-grid-row fr-grid-row--gutters">
              <div className="fr-col-12 fr-col-md-6">
                <p className="fr-text--sm fr-text-mention--grey fr-mb-1v">Traité le</p>
                <p className="fr-mb-0">
                  {focusedTicket.processedAt
                    ? dateTimeFormatter.format(new Date(focusedTicket.processedAt))
                    : "—"}
                </p>
              </div>
              <div className="fr-col-12 fr-col-md-6">
                <p className="fr-text--sm fr-text-mention--grey fr-mb-1v">Traité par</p>
                <p className="fr-mb-0">{formatUser(focusedTicket.processedBy ?? null)}</p>
              </div>
            </div>
            ...
            <div className="fr-grid-row fr-grid-row--gutters">
              <div className="fr-col-12">
                <div className="fr-btns-group fr-btns-group--inline fr-btns-group--icon-left">
                  <Button
                    priority={focusedTicket.status === "PENDING" ? "primary" : "secondary"}
                    iconId={
                    ...
                    onClick={() =>
                      handleStatusChange(
                        focusedTicket,
                        focusedTicket.status === "PENDING" ? "RESOLVED" : "PENDING",
                      )
                    }
                    disabled={isPending}
                  >
